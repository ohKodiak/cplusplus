...
Image::Image(const Image& obj): Image(){
  allocateImage(obj.width, obj.height);;
  for(unsigned int row = 0; row < height; row++){
    for(unsigned int col = 0; col< width; col++){
      Pixel px = obj.image[col][row];
      image[col][row].setRed(px.getRed());
      image[col][row].setGreen(px.getGreen());
      image[col][row].setBlue(px.getBlue());
    }}}
...
Image& Image::operator= (const Image& lhs) {
  ...
  for (unsigned int col = 0; col < lhs.width; ++col) {
    image[col] = new Pixel[lhs.height];}

  for (unsigned int row = 0; row < height; row++) {
    for (unsigned int col = 0; col < width; col++) {
      image[col][row].setRed(lhs.image[col][row].getRed());
      image[col][row].setGreen(lhs.image[col][row].getGreen());
      image[col][row].setBlue(lhs.image[col][row].getBlue());
    }}
  return *this;}
